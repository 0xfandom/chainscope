...
async fn run_then_kill(pool: &PgPool, kill_after: Duration) {
    ...
    let (raw_sink, raw_source) =
        chainscope_core::build_transport::<chainscope_core::Envelope<BlockUnit>>(chainscope_core::TransportSpec::Channel { capacity: 32 }).unwrap();
    let (row_sink, row_source) =
        chainscope_core::build_transport::<chainscope_core::Envelope<RowBatch>>(chainscope_core::TransportSpec::Channel { capacity: 32 }).unwrap();

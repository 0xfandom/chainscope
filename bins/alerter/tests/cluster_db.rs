//! Cluster-buy detector (#102), against a real Postgres.
//!
//!   DATABASE_URL=postgres://chainscope:chainscope@localhost:5432/chainscope \
//!     cargo test -p chainscope-alerter --test cluster_db -- --ignored --nocapture
...
use chainscope_alerter::detect::cluster_buys;
...
async fn fresh_db(admin: &PgPool, watched: &[Address20]) -> (PgPool, String) {
    let name = format!("chainscope_cluster_{}", std::process::id());
...
/// `wallet` buys TOKEN at `block`, `secs` after the base time.
async fn buy(pool: &PgPool, block: i64, tx_byte: u8, wallet: Address20, secs: i64) {
    let mut tx = [0u8; 32];
    tx[0] = tx_byte;
    ...
    sqlx::query(
        "INSERT INTO swaps (block_time, tx_hash, log_index, block_number, pool, sender, \
         ...
         VALUES (to_timestamp($1), $2, 0, $3, $4, $5, $6, -2000000000000000000, 300000000, 1, 1, 0)",
    )
    .bind(BASE_TS + secs)
    .bind(tx.as_slice())
    .bind(block)
...
async fn three_watched_wallets_buying_one_token_cluster() {
    let Some(admin) = admin().await else { return };
    let (pool, name) = fresh_db(&admin, &[W1, W2, W3]).await;
    ...
    // Three distinct watched wallets buy TOKEN within two minutes.
    buy(&pool, 900, 0xB1, W1, 0).await;
    buy(&pool, 901, 0xB2, W2, 60).await;
    buy(&pool, 902, 0xB3, W3, 120).await;

    assert_eq!(cluster_buys(&alerter).await.unwrap(), 1, "three wallets, one cluster");
    assert_eq!(sends.load(Ordering::SeqCst), 1);
    // A growing cluster (a re-scan) keeps the same key — no re-alert.
    assert_eq!(cluster_buys(&alerter).await.unwrap(), 0);
    assert_eq!(sends.load(Ordering::SeqCst), 1);
...
#[tokio::test]
#[ignore = "requires DATABASE_URL"]
async fn two_wallets_do_not_cluster() {
    let Some(admin) = admin().await else { return };
    let (pool, name) = fresh_db(&admin, &[W1, W2, W3]).await;
    let sends = Arc::new(AtomicUsize::new(0));
    let alerter = Alerter {
    ...
    assert_eq!(cluster_buys(&alerter).await.unwrap(), 0, "two is below cluster_size");
